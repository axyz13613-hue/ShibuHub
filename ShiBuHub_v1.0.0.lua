--[[
    ShiBuHub v1.0.0 BRANDED • RARITY PICKER
    ...
    NOTE: Event Auto Feed is intentionally not enabled until AskFeed payload is confirmed.
]]
...
local function tryCarry(uid)
    -- Current servers usually accept {Uid = uid}; some builds accept the raw uid.
    local ok,res=pcall(function() return RF_EggCarry:InvokeServer({Uid=uid}) end)
    if not ok then
        local ok2,res2=pcall(function() return RF_EggCarry:InvokeServer(uid) end)
        return ok2,res2,"raw"
    end

    task.wait(.16)
    local still=fieldStillHas(uid)
    if still==true then
        local ok2,res2=pcall(function() return RF_EggCarry:InvokeServer(uid) end)
        if ok2 then return true,res2,"raw-fallback" end
    end
    return true,res,"table"
end

local function carryEgg(target)
    if not target or not target.CFrame or not RF_EggCarry then return false end
    local _,root=characterRoot()
    local uid=target.Record.Uid
    CurrentTarget = target.Rarity .. " / " .. tostring(uid)
    LastAction = "TP -> "..target.Rarity

    -- Re-resolve right before moving, because the server can spawn the record before the model appears.
    local liveCF=recordCFrame(target.Record) or target.CFrame
    root.CFrame = liveCF * CFrame.new(0,2.35,0)
    task.wait(.16)

    local ok,res,shape=tryCarry(uid)
    if not ok then
        LastAction="Carry failed"
        return false
    end

    task.wait(.22)
    if fieldStillHas(uid)==true then
        -- One short re-approach catches eggs whose hit/carry radius is strict.
        local retryCF=recordCFrame(target.Record) or liveCF
        root.CFrame = retryCF * CFrame.new(0,1.2,0)
        task.wait(.12)
        ok,res,shape=tryCarry(uid)
        task.wait(.18)
    end

    if fieldStillHas(uid)==true then
        LastAction="Reached "..target.Rarity.." but carry was rejected"
        return false
    end

    LastAction = "Carried "..target.Rarity.." ("..tostring(shape)..")"

    if Config.AutoReturnBase and BaseCF then
        root.CFrame = BaseCF * CFrame.new(0,2,0)
        task.wait(.7)
        if RF_Doff then pcall(function() RF_Doff:InvokeServer() end) end
    end
...
local hl=textLabel(homeCard,"ShiBuHub v1.0.0",22,true)
...
local h4=textLabel(homeCard,"Use EGGS to save your base and start Auto Steal.",12,false)
...
local _,setAuto,getAuto=toggleRow(Eggs,190,"Auto Steal","Automatically steal selected rarities",false,function(v)
    Config.AutoSteal=v
end)
...
local _,setReturn=toggleRow(Eggs,424,"Auto Return Base","Return to your saved base after carry",true,function(v)
    Config.AutoReturnBase=v
end)

local priorityCard=card(Eggs,502,112)
...
saveBaseBtn.Position=UDim2.fromOffset(16,628)
...
local ie=textLabel(infoEvent,"AUTO FEED",15,true)
ie.Position=UDim2.fromOffset(14,10); ie.Size=UDim2.new(1,-28,0,25); ie.TextColor3=C.GOLD
local ie2=textLabel(infoEvent,"Waiting for exact AskFeed payload confirmation. Snapshot monitoring is active and safe to use.",12,false)
...
pageHeader(Webhook,"WEBHOOK","Optional notification configuration")
local whCard=card(Webhook,80,150)
local whInfo=textLabel(whCard,"Webhook is left disabled in this build. The field below is UI-ready for a later notification module.",12,false)
whInfo.Position=UDim2.fromOffset(16,12); whInfo.Size=UDim2.new(1,-32,0,46); whInfo.TextWrapped=true; whInfo.TextColor3=C.MUTED
local whBox=Instance.new("TextBox")
whBox.Size=UDim2.new(1,-32,0,42); whBox.Position=UDim2.fromOffset(16,82); whBox.BackgroundColor3=C.PANEL2; whBox.TextColor3=C.WHITE; whBox.PlaceholderText="Webhook URL"; whBox.PlaceholderColor3=C.MUTED; whBox.Text=""; whBox.ClearTextOnFocus=false; whBox.Font=Enum.Font.Gotham; whBox.TextSize=12; whBox.Parent=whCard
...
task.spawn(function()
    while Gui.Parent and G.ShiBuHubRunId==RUN_ID do
        ...
        if Config.AutoSteal then
            ...
            if not ok then LastAction="Error: "..tostring(err) end
        end
    end
end)
...
setPage("EGGS")
refreshMonsterState()
print("[ShiBuHub] v1.0.0 RARITY PICKER loaded")
