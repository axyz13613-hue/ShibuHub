...
version.Text="v2.0.3 • Monster Auto Feed"
version.ZIndex=5002
version.Parent=card
...
local SOURCE = [======[
--[[
    ShiBuHub v2.0.3 • COMPILE FIX + MONSTER AUTO FEED
...
local homeCard=card(Home,80,180)
local hl=textLabel(homeCard,"ShiBuHub v2.0.3",22,true)
hl.Position=UDim2.fromOffset(18,16); hl.Size=UDim2.new(1,-36,0,32); hl.TextColor3=C.CYAN
local h2=textLabel(homeCard,"Egg farming engine + Hungry Monster monitor",13,false)
h2.Position=UDim2.fromOffset(18,54); h2.Size=UDim2.new(1,-36,0,24); h2.TextColor3=C.MUTED
local h3=textLabel(homeCard,"Rarity picker: choose any rarity from Common to Eternal",14,true)
h3.Position=UDim2.fromOffset(18,92); h3.Size=UDim2.new(1,-36,0,28); h3.TextColor3=C.TEAL
local h4=textLabel(homeCard,"Choose rarities, enable Auto Run + Pick + Return, and Evasive Run bends around nearby animals.",12,false)
...
local StatusCard=card(Eggs,74,102)
local statusTitle=textLabel(StatusCard,"STATUS",12,true)
statusTitle.Position=UDim2.fromOffset(14,8); statusTitle.Size=UDim2.new(1,-28,0,20); statusTitle.TextColor3=C.CYAN
local statusText=textLabel(StatusCard,"Ready • scanning eggs",15,true)
statusText.Position=UDim2.fromOffset(14,31); statusText.Size=UDim2.new(1,-28,0,24); statusText.TextColor3=C.GREEN
local statusMeta=textLabel(StatusCard,"Scanned: 0   •   Target: None",11,false)
statusMeta.Position=UDim2.fromOffset(14,63); statusMeta.Size=UDim2.new(1,-28,0,22); statusMeta.TextColor3=C.MUTED

local _,setAuto,getAuto=toggleRow(Eggs,190,"Auto-Steal","Run to selected egg, pick it, then return to base",false,function(v)
...
local _,setPersistent=toggleRow(Eggs,268,"Persistent Steal","Continue scanning after respawn",true,function(v)
    Config.PersistentSteal=v
end)
local _,setTrap=toggleRow(Eggs,346,"Prevent Traps","Skip trap/cage/snare egg models",true,function(v)
    Config.PreventTraps=v
end)
local _,setEvasive=toggleRow(Eggs,424,"Evasive Run","Curve away from nearby NPC animals while moving",true,function(v)
    Config.EvasiveRun=v
end)
local _,setReturn=toggleRow(Eggs,502,"Auto Return Base","After pickup, automatically run back to saved base",true,function(v)
    Config.AutoReturnBase=v
end)

local _,_,getESP=toggleRow(Eggs,580,"Egg ESP","Highlight currently selected egg rarities",false,function(v)
    Config.EggESP=v
end)
local _,_,getHatch=toggleRow(Eggs,658,"Auto Hatch","Try to hatch/finish owned eggs automatically",false,function(v)
    Config.AutoHatch=v
end)

local priorityCard=card(Eggs,736,112)
local pt=textLabel(priorityCard,"RARITY FILTER",12,true)
pt.Position=UDim2.fromOffset(14,8); pt.Size=UDim2.new(1,-28,0,20); pt.TextColor3=C.CYAN

local selectedInfo=textLabel(priorityCard,"Selected: "..targetSummary(),11,false)
...
local openRarityBtn=button(priorityCard,"◉  SELECT RARITIES")
...
local rarityShade=Instance.new("Frame")
...
local rarityModal=Instance.new("Frame")
...
local rarityTitle=textLabel(rarityModal,"◉  RARITIES",22,true)
...
local doneRarity=button(rarityModal,"DONE")
...
local searchHint=textLabel(rarityModal,"Choose any egg rarity to test Auto Steal",12,false)
...
local allBtn=button(rarityModal,"ALL")
allBtn.Size=UDim2.fromOffset(94,34)
allBtn.Position=UDim2.new(1,-214,0,88)
allBtn.ZIndex=1002

local noneBtn=button(rarityModal,"NONE")
...
local saveBaseBtn=button(Eggs,"⌂  SAVE CURRENT POSITION AS BASE")
...
local progCard=card(Progression,80,112)
local progTitle=textLabel(progCard,"PROGRESSION TOOLS",16,true)
progTitle.Position=UDim2.fromOffset(16,10); progTitle.Size=UDim2.new(1,-32,0,26); progTitle.TextColor3=C.CYAN
local progStatus=textLabel(progCard,"Ready",12,false)
...
local collectAway=button(Progression,"COLLECT AWAY EARNINGS")
...
local redeemCodex=button(Progression,"REDEEM CODEX ALL")
...
local wearBest=button(Progression,"WEAR BEST")
...
local treadState=button(Progression,"READ TREADMILL")
...
treadState.MouseButton1Click:Connect(function()
    local ok,res=invokeRemote(RF_TreadRender)
    progStatus.Text=ok and ("Treadmill snapshot: "..tostring(type(res))) or "Treadmill snapshot unavailable."
end)

local treadRaise=button(Progression,"UPGRADE TREADMILL")
treadRaise.Size=UDim2.new(.5,-22,0,42); treadRaise.Position=UDim2.new(.5,6,0,366)
treadRaise.MouseButton1Click:Connect(function()
    local ok=invokeRemote(RF_TreadRaise)
    progStatus.Text=ok and "Treadmill upgrade request sent." or "Treadmill upgrade unavailable."
end)

local _,_,getAutoAway=toggleRow(Progression,430,"Auto Collect Away","Collect away earnings periodically",false,function(v)
    Config.AutoCollectAway=v
end)
local _,_,getAutoBest=toggleRow(Progression,508,"Auto Wear Best","Periodically request the best haul equipment",false,function(v)
    Config.AutoWearBest=v
end)
local _,_,getAutoCodex=toggleRow(Progression,586,"Auto Redeem Codex","Redeem available codex rewards periodically",false,function(v)
...
local monsterCard=card(Event,80,180)
local mt=textLabel(monsterCard,"THE HUNGRY MONSTER",17,true)
mt.Position=UDim2.fromOffset(16,12); mt.Size=UDim2.new(1,-32,0,28); mt.TextColor3=C.CYAN
local monsterStatus=textLabel(monsterCard,"Reading snapshot...",13,false)
...
local infoEvent=card(Event,278,108)
local ie=textLabel(infoEvent,"AUTO FEED • CONFIRMED",15,true)
ie.Position=UDim2.fromOffset(14,10); ie.Size=UDim2.new(1,-28,0,25); ie.TextColor3=C.GOLD
local ie2=textLabel(infoEvent,"AskFeed takes no arguments. The server consumes the currently held egg and returns Charge / PendingChests / EggUid.",12,false)
ie2.Position=UDim2.fromOffset(14,38); ie2.Size=UDim2.new(1,-28,0,55); ie2.TextWrapped=true; ie2.TextColor3=C.MUTED

local _,_,getAutoFeedMonster=toggleRow(Event,402,"Auto Feed Hungry Monster","Pick allowed field eggs, carry them to the monster, then feed automatically",false,function(v)
    Config.AutoFeedMonster=v
end)

local _,_,getPriorityMonster=toggleRow(Event,480,"Priority Hungry Monster","Pause normal Auto-Steal while event Auto Feed is working",true,function(v)
    Config.PriorityHungryMonster=v
end)

local _,_,getProtectEvent=toggleRow(Event,558,"Protect Rarities","Never feed rarities selected in the EGGS rarity picker",true,function(v)
    Config.ProtectSelectedRarities=v
end)

local feedCard=card(Event,636,112)
local feedTitle=textLabel(feedCard,"FEED RARITIES",12,true)
feedTitle.Position=UDim2.fromOffset(14,8); feedTitle.Size=UDim2.new(1,-28,0,20); feedTitle.TextColor3=C.CYAN
local feedInfo=textLabel(feedCard,"Feed: "..eventFeedSummary(),11,false)
...
local openFeedRarity=button(feedCard,"✦  SELECT FEED RARITIES")
...
local manualFeed=button(Event,"FEED CURRENTLY HELD EGG")
...
local protectCard=card(Event,820,92)
local protectText=textLabel(protectCard,"Protected: "..targetSummary(),12,false)
...
local feedShade=Instance.new("Frame")
...
local feedModal=Instance.new("Frame")
...
local fmRows={}
local function updateFeedRarityUI()
...
local sellCard=card(Sell,80,130)
local sellInfo=textLabel(sellCard,"Use the game's Haul sale remote for your current satchel.",13,false)
...
local sellNow=button(sellCard,"SELL SATCHEL NOW")
...
local _,_,getAutoSell=toggleRow(Sell,228,"Auto Sell Satchel","Offer the current satchel for sale periodically",false,function(v)
    Config.AutoSellSatchel=v
end)

local sellPets=button(Sell,"SELL EVERY PET")
...
local sellProtect=card(Sell,372,104)
local sp1=textLabel(sellProtect,"SELL FILTER / PROTECTION",14,true)
sp1.Position=UDim2.fromOffset(14,10); sp1.Size=UDim2.new(1,-28,0,24); sp1.TextColor3=C.CYAN
local sp2=textLabel(sellProtect,"Selected rarities: "..targetSummary().."\nUse the EGGS rarity picker as your keep/protect list.",12,false)
...
local whCard=card(Webhook,80,184)
local whInfo=textLabel(whCard,"Optional Discord webhook for ShiBuHub notifications.",12,false)
...
local whTest=button(whCard,"SAVE + TEST")
...
whTest.MouseButton1Click:Connect(function()
    ...
    local ok,why=sendWebhook("ShiBuHub connected","Webhook test from ShiBuHub v2.0.3")
    whTest.Text=ok and "✓ TEST SENT" or ("FAILED • "..tostring(why))
end)

local _,_,getWebhook=toggleRow(Webhook,282,"Enable Webhook","Allow notifications to the URL above",false,function(v)
    Config.WebhookUrl=whBox.Text
    Config.WebhookEnabled=v
end)
local _,_,getFoundNotify=toggleRow(Webhook,360,"Notify Target Found","Send a notification when a selected target appears",false,function(v)
    Config.NotifyTargetFound=v
end)
local _,_,getPickedNotify=toggleRow(Webhook,438,"Notify Target Picked","Send a notification after pickup is confirmed",true,function(v)
...
local _,_,getAnti=toggleRow(Settings,80,"Anti AFK","Prevent idle disconnect",true,function(v) Config.AntiAFK=v end)
local _,_,getFPS=toggleRow(Settings,158,"Low FPS Mode","Reduce graphics and cap FPS to 15",false,function(v)
    Config.FPSBoost=v
    if v then
...
local _,_,getRejoin=toggleRow(Settings,236,"Auto Rejoin","Rejoin after Roblox disconnect prompt",true,function(v) Config.AutoRejoin=v end)

local movementCard=card(Settings,314,116)
local mv1=textLabel(movementCard,"MOVEMENT",14,true)
mv1.Position=UDim2.fromOffset(14,10); mv1.Size=UDim2.new(1,-28,0,22); mv1.TextColor3=C.CYAN
local mv2=textLabel(movementCard,"Pathfinding + Humanoid:MoveTo using the game-provided WalkSpeed. Direct CFrame teleport and WalkSpeed overrides are disabled.",12,false)
...
task.spawn(function()
    while Gui.Parent and G.ShiBuHubRunId==RUN_ID do
        ...
        if Config.AutoSteal and not FarmBusy and not (Config.AutoFeedMonster and Config.PriorityHungryMonster and monsterIsActive()) then
            FarmBusy=true
