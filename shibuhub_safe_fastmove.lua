...
local function walkToPosition(destination, stopDistance, timeout)
    ...
    if not hum or not root then return false,"no-humanoid" end

    stopDistance = stopDistance or Config.FastStepStopDistance or 5.5
    timeout = timeout or 30

    local function closeEnough()
        return root.Parent and (root.Position-destination).Magnitude <= stopDistance
    end
    ...
    while Config.AutoSteal and os.clock()-started < timeout do
        ...
        if now>=nextProgress then
            local moved=(root.Position-lastPos).Magnitude
            stuck = moved<1.0 and (stuck+1) or 0
            ...
            if stuck>=2 then
                buildPath()
                stuck=0
            elseif Config.FastMoveMode then
                -- Prefer normal direct Humanoid movement when progress is good.
                waypoints=nil
                wpIndex=1
            end
        end
        ...
        if not Config.FastMoveMode and not waypoints then
            buildPath()
        end

        if waypoints and waypoints[wpIndex] then
            local wp=waypoints[wpIndex]
            if (root.Position-wp.Position).Magnitude<4.5 then
                wpIndex+=1
        ...
        if dodging and math.random()<.10 then
            hum.Jump=true
        end
...
_,_,getFastMove=toggleRow(Settings,448,"Fast Move","Ưu tiên Humanoid:MoveTo trực tiếp; tự fallback pathfinding khi bị kẹt",true,function(v)
    Config.FastMoveMode=v
end)
