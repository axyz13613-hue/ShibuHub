--[[
    ShiBuHub v1.0.5 BRANDED • LEMON SPEED
...
local function walkToPosition(destination, stopDistance, timeout)
    ...
    local requestedSpeed = tonumber(Config.FastStepSpeed) or 36

    -- Lemon Speed mode: faster Humanoid movement only; no direct HRP.CFrame teleport.
-- Exact LennonHub internals are unknown because its public source is obfuscated.
    -- Some games may still reject client-side WalkSpeed changes, so the
    -- speed is restored after each trip.
    pcall(function()
        if requestedSpeed > originalSpeed then
            hum.WalkSpeed = requestedSpeed
        end
    end)
...
local function walkBackToBase()
    ...
    LastAction="Lemon Speed returning to base"
...
local function carryEgg(target)
    ...
    LastAction = "Lemon Speed -> "..target.Rarity
...
local hl=textLabel(homeCard,"ShiBuHub v1.0.5",22,true)
...
local _,setAuto,getAuto=toggleRow(Eggs,190,"Lemon Speed Farm","Fast run to selected egg, pick it, then fast return to base",false,function(v)
...
local priorityCard=card(Eggs,580,112)
...
task.spawn(function()
    while Gui.Parent and G.ShiBuHubRunId==RUN_ID do
        ...
        if Config.AutoSteal and not FarmBusy then
            FarmBusy=true
...
setPage("EGGS")
refreshMonsterState()
print("[ShiBuHub] v1.0.5 LEMON SPEED loaded")
